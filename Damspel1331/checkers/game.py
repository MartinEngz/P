...
class Game:
    ...
    def select_piece(self, row, col):
        ...
        if piece in valid_pieces:
            self.selected = piece
            if self.board.get_valid_skips(self.selected):
                self.valid_moves = self.board.get_valid_skips(self.selected) # change to path for debugging (self.board.get_valid_skips() for working program)
                # print('yes') # for debugging 
            else:
                self.valid_moves = self.board.get_valid_steps(self.selected)
    ...
    def select_move(self, row, col):
        ...
        if (row, col) in self.valid_moves:
            ...
            self.has_skipped = self.valid_moves[(row, col)]
            if self.has_skipped:
                self.board.remove(self.has_skipped)
                self.valid_moves = self.board.get_valid_skips(self.selected)
                if not self.valid_moves:
                    self.has_completed_move = True
            else:
                self.has_completed_move = True
